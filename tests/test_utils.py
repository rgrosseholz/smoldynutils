...
def test_gaussian_probability():
    ...
    assert np.isclose(gauss_probability_density(-1, mu, sigma), gauss_probability_density(1, mu, sigma))
...
def test_theoretical_brownian_motion():
    ...
    assert np.isclose(theoretical_brownian_motion_pdf(x, D, t), gauss_probability_density(x, 0, sigma))
